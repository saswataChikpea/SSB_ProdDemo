...
import getJiraClouldIdLWC from '@salesforce/apex/JiraConnectApexDevelpmentLwc.getJiraClouldIdLWC'
import createJiraTicketLWC from '@salesforce/apex/JiraConnectApexDevelpmentLwc.createJiraTicketLWC'
import createJiraProjectLWC from '@salesforce/apex/JiraConnectApexDevelpmentLwc.createJiraProjectLWC'
import getJiraTicketLWC from '@salesforce/apex/JiraConnectApexDevelpmentLwc.getJiraTicketLWC'
import jiraGenericCallLWC from '@salesforce/apex/JiraConnectApexDevelpmentLwc.jiraGenericCallLWC'
// import getFieldApiNames from '@salesforce/apex/JiraConnectApexDevelpmentLwc.getFieldApiNames'
import LightningConfirm from 'lightning/confirm';
import LightningAlert from 'lightning/alert';

export default class JiraConnectDevelopmentLwc extends LightningElement {
    ...
    firstTimeIssueCreationFailed=false;
    ...
    accountId = "63284dc4a84c7f79c3832612" // 557058:1f2502f3-6b58-458d-98f5-346deb28b902 (Adam K.| Admin)
    totalTask = {
    ...
    @wire(getRelatedListRecords, {
        ...
        fields: ['ChikPeaSSB__Project_Phase__c.Name', 'ChikPeaSSB__Project_Phase__c.ChikPeaSSB__Dependencies__c',
            ...
            'ChikPeaSSB__Project_Phase__c.ChikPeaSSB__Est_Start_date__c', 'ChikPeaSSB__Project_Phase__c.ChikPeaSSB__Estimated_Hours__c', 
            'ChikPeaSSB__Project_Phase__c.ChikPeaSSB__Est_End_Date__c'],
    ...
    solveDependency(){
        ...
        this.phase_records.forEach(e => {
            let dependency_dict = {
                name: e.fields.Name.value,
                dependencies: e.fields.ChikPeaSSB__Dependencies__c.value,
                key: ""
            }
            dependency_array.push(dependency_dict)
        })

        console.log('JSON.stringify(dependency_array)');
        console.log(JSON.stringify(dependency_array));
        
        // Rearrange phase/Issues as per dependencies
        let reArrangedPhases = this.rearrangeIssues(dependency_array);  
        console.log('JSON.stringify(reArrangedPhases)');
        console.log(JSON.stringify(reArrangedPhases));
        
    }

    // Rearrange phase/Issues as per dependencies
    rearrangeIssues(array) {
        let projectKey = 'PRJK0001';
        const result = [];
        const independent = array.filter(obj => obj.dependencies === "Independent");
        const dependent = array.filter(obj => obj.dependencies !== "Independent");
        let count = 1;
        while (independent.length > 0) {
          const issue = independent.shift();
          if (!result.some(obj => obj.name === issue.name)) {
            
            issue.key = `${projectKey}-${count++}`;
            result.push(issue);
          }
          dependent.forEach((obj, index) => {
            if (obj.dependencies === issue.name) {
              if (!result.some(o => o.name === obj.name)) {
                obj.key = `${projectKey}-${count++}`;
                result.push(obj);
              }
              dependent.splice(index, 1);
              independent.push(obj);
            }
          });
        }
      
        if (dependent.length > 0) {
          console.warn(`Missing dependencies: ${dependent.map(obj => obj.dependencies).join(", ")}`);
    ...
    resolveDependency() {
        ...
        let index = 1;
        // let index = this.phase_records.length;

        console.log('Phase records length: ');
        console.log(this.phase_records.length);
        this.solveDependency();
        console.log('JSON.stringify(this.phase_records)');
        console.log(JSON.stringify(this.phase_records));
    ...
    startSync() {
        ...
        if (this.totalTask.gotJiraClouldIdLWC && 
            ...
            this.totalTask.gotRelatedListRecords) {
                console.log('this.totalTask.jiraProjectCreated ====> ',this.totalTask.jiraProjectCreated);
            if (!this.totalTask.jiraProjectCreated) {
                //if(this.countNoOfTimesCreateJiraApiCalled == 0){
                    console.log('Create Jira Project & issue ',this.countNoOfTimesCreateJiraApiCalled);
                    this.createJiraProject();
                    console.log('firstTimeIssueCreationFailed =>',this.firstTimeIssueCreationFailed);
                    if(this.firstTimeIssueCreationFailed){
                        this.firstTimeIssueCreationFailed=false;
    ...
    createJiraIssue() {
        ...
        this.issue_body.issueUpdates = reversed.map(element => {
            ...
            issue1.fields.description.content[0].content[0].text = element.fields.ChikPeaSSB__Descriptions__c.value
            // issue1.fields.timetracking.originalEstimate = element.fields.ChikPeaSSB__Estimated_Hours__c;    // Added
            ...
            //issue link
            console.log('Issue links are disabled. 197');
            // console.log(this.phase_dependency);
            if (this.phase_dependency[element.fields.Name.value]) {
                issue1.update.issuelinks = []
                //issue1.fields.key = element.key
                this.phase_dependency[element.fields.Name.value].forEach(e => {
        ...
        createJiraTicketLWC({ 
            ...
            console.log(' => Create issue result indepnedent OR dependent =>', JSON.stringify(result,null,'\t'))
            if (result.StatusCode === 201) {
                ...
                this.firstTimeIssueCreationFailed=true;
                const fieldErrorsKeys = Object.keys(result.errors)
                let errorMsg1;
                const errorMsg = fieldErrorsKeys.map(e => {
                    const fieldLabel = e
    ...
    getJiraTicketLWCJS() {
        ...
        getJiraTicketLWC({ 
            ...
            console.log('getJiraTicketLWC Success=>', JSON.stringify(data,null,'\t'));
        }).catch(error => {
    ...
    jiraGenericCallLWCJS() {
        ...
        jiraGenericCallLWC({ 
            ...
            console.log('jiraGenericCallLWCJS Success=>', JSON.stringify(data,null,'\t'));
        }).catch(error => {
