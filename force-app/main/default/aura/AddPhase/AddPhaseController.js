({
    ...
    ClonePhase: function (component, event, helper) {
        console.log('ClonePhase called: ');
        var phaseRecId = event.target.id;
        console.log('phaseId: ', phaseRecId);

        // var msg = 'Are you sure you want to Clone this Phase?';
        // if (!confirm(msg)) {                // Try adding lightning Alart Box
        //     console.log('No need to clone.');
        //     return false;
        // } else {
            var recID = component.get("v.recordId");
            console.log('recID: ', recID);
            var action = component.get("c.clonePhaseDetails");     // class need to be configured

            action.setParams({
                recordId: recID,
                phaseId: phaseRecId
            });
            console.log('ClonePhase action: ', action);
            action.setCallback(this, function (response) {
                console.log('action.setCallback: ', response);

                var state = response.getState();
                var data = response.getReturnValue();
                if (state == "SUCCESS") {
                    console.log('Phase Cloned Successfully');
                    var toastEvent = $A.get("e.force:showToast");
                    toastEvent.setParams({
                        message: 'Phase Cloned succesfully',
                        messageTemplate: 'Record {0} created! See it {1}!',
                        duration: '5000',
                        key: 'info_alt',
                        type: 'success',
                    ...
                    toastEvent.fire();
                    console.log('Error while cloning:  ', errors);  
                }
            });
            $A.enqueueAction(action);
            console.log('$A.enqueueAction(action): called.');
        // }
    },
})